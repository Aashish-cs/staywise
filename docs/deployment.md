...
2. Add environment variables:
   ...
   - `STAYWISE_ENABLE_STRIPE_CHECKOUT` (optional, server-only; set to `true` only with Stripe test credentials and the payment-required reservation migration)
   - `STRIPE_SECRET_KEY` (optional, server-only; required for hosted Checkout when payment-required reservations are enabled)
