...
import { CalendarDays, Home, Plus, Sparkles, Star, WalletCards } from "lucide-react";
...
export default async function HostPage() {
  ...
  const hostListings = staywiseListings.slice(0, 4);
    ...
    <main className="min-h-screen bg-[#f7f3ee] text-[#201a18]">
      <header className="border-b border-[#eadfd6] bg-[#fffaf5]">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <Link href="/auth" className="rounded-full bg-[#201a18] px-4 py-2 text-sm font-semibold text-white">
            {user ? "Account" : "Host sign in"}
          </Link>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-7xl px-5 py-8 lg:px-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <button className="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-[#ff385c] px-5 text-sm font-semibold text-white hover:bg-[#df2348]">
        ...
        <div className="mt-7 grid gap-4 md:grid-cols-3">
          <HostMetric icon={Home} label="Active listings" value="4" />
          <HostMetric icon={CalendarDays} label="Upcoming reservations" value="12" />
          <HostMetric icon={WalletCards} label="Projected revenue" value="$8.4k" />
        </div>
          ...
          <section className="rounded-[24px] border border-[#eadfd6] bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 divide-y divide-[#eadfd6]">
              {hostListings.map((listing) => (
                <article key={listing.id} className="grid gap-4 py-5 md:grid-cols-[128px_1fr_auto] md:items-center">
                  <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-[#e8dfd6]">
                    <Image
