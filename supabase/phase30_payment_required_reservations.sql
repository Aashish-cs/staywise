-- StayWise Phase 30 payment-required reservation path.
-- Run this after phase33_pricing.sql when Stripe test mode should be enabled.
-- The existing create_reservation RPC stays pay-later; this separate RPC is
-- called only when the server has intentionally enabled Stripe checkout.

create or replace function public.create_payment_required_reservation(
...
grant execute on function public.create_payment_required_reservation(uuid, date, date, integer) to authenticated;
